...
public abstract class KMeansClustering<T> {
    ...
    /**
     * @param preCenter 聚类中心
     * @param times     当前已聚类次数
     * @Description: 一轮聚类，递归进行下一轮
     */
    private void clustering(List<T> preCenter, int times) {
        ...
        // 二维集合，一维存放每个质心list，二维存放每个质心对应的数据对象
    ...
    /**
     * @param size 质心数量
     * @Description: 初始化二维List集合，用来存放聚类结果
     */
    ...
    /**
     * @param lists 存放结果的二维List集合
     * @Description: 清空存放结果的二维List集合
     */
    ...
    /**
     * @param value 需要添加的新数据
     * @Description: 向数据集中添加新数据
     */
    ...
    private boolean isCenterChange(List<T> preT, List<T> nowT) {
        ...
        // 前一组的质心是否在后一个质心组中出现，有一个没有出现，就认为质心发生了变动
        for (T t1 : preT) {
            boolean bol = true;
            for (T t2 : nowT) {
